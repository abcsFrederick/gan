...
def cleanup():
    ...
    fileList = glob.glob('mnist_64*.png')
...
class MNIST_DCGAN(object):
    def __init__(self):
        ...
        self.x_train = load_data('data/cxr8_resized/64/infiltrationAP') # Change to Path to Image directory
    ...
    def plot_images(self, save2file=False, fake=True, samples=16, noise=None, step=0):
        filename = 'chestxray_64.png'
        if fake:
            filename = 'chestxray_fake_64.png'
            if noise is None:
                noise = np.random.uniform(-1.0, 1.0, size=[samples, 100])
            else:
                filename = "mnist_64_%04d.png" % step
...
if __name__ == '__main__':
    ...
    #uncomment the following statement to save a snapshot of real images
